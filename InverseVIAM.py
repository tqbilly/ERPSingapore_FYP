...
xLen=EstimateDemandAM.xLen
freespeed=EstimateDemandAM.freespeed
speed=EstimateDemandAM.speed

# parameter N: 9*14
N=EstimateDemandAM.N
...
# parameter ta0: 14*1
t0=EstimateDemandAM.t0

# parameter d: 9*8
d=EstimateDemandAM.d
d=np.matrix(d)
dmat=d.reshape(9,8)
...
zlist = EstimateDemandAM.zlist
t1 = EstimateDemandAM.flowt
m = EstimateDemandAM.m
...
resWithoutERP = InverseVINT.myres
error = 1e9
...
for i in range(0, 30):
    temp = 0
    for j in range(0, 6):
        temp += resWithoutERP[j] * (x[i] ** j)
    yWithoutERP.append(temp)

def errorEsti(el,erp):
    error=0
    res = InvVI(const,deg,lamb,erp,el)
    x = np.linspace(0, 1, 30)
    y = []
    for i in range(0, 30):
        temp = 0
        ...
        y.append(temp)
    for i in range(0,30):
        error += abs(yWithoutERP[i]+erp-y[i])
    return error

erpSpace = [0.0005, 0.001, 0.0015, 0.0020, 0.0025]
err=1e9
erp_choice=0.0005
for i in range(5):
    err_i = errorEsti(get_elist(0,300),erpSpace[i])
    if(err_i<=err):
        err = err_i
        erp_choice = erpSpace[i]
print erp_choice
myres = InvVI(1.2,5,100,erp_choice,get_elist(0,300))